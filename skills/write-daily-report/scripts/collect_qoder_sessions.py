...
def main() -> int:
    ...
    parser.add_argument("--qoder-home", default=None,
                        help="Qoder data dir (default: $QODER_HOME or "
    ...
    qoder_home = (
        Path(args.qoder_home).expanduser().resolve()
        if args.qoder_home
        else resolve_app_home("QODER_HOME", "Qoder")
    )

    sessions: list[dict[str, Any]] = []
    start_mtime = start.timestamp()

    for home in candidate_workspace_homes(qoder_home):
    ...
    emit_output(
        ...
        extra={"qoderHome": str(qoder_home)},
    )
    return 0
