...
- 同一模块按领域将状态、派生值、操作和处理器相邻放置，并复用稳定前缀，例如 `paymentItems`、`paymentTotal`、`loadPaymentItems`、`handlePaymentRetry`。使用简短的分组注释标识不显而易见的区块；不要为名字已清晰的每个变量或函数重复写注释。对复杂约束、业务原因、兼容处理或非直觉副作用，补一行说明“为什么”，而非复述“做什么”。
